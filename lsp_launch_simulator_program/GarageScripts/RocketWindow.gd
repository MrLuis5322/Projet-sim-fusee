...
func _ready():
	...
	var timer = Timer.new()
	timer.wait_time = 5.0  # 5 secondes
	timer.one_shot = true
	add_child(timer)
...
func _on_Timer_timeout():
	# Charger la texture que vous souhaitez afficher à travers la vitre après 5 secondes
	self.texture = preload("res://zImages/Window.jpg")  # Remplacez par le chemin de votre image
